...
def enviar_processed_para_minio() -> bool:
    if not PROCESSED_DIR.exists():
        logger.warning(f"Pasta '{PROCESSED_DIR}' não encontrada. Nada para enviar.")
        return False

    arquivos = [f for f in PROCESSED_DIR.rglob("*") if f.is_file() and f.name != ".gitkeep"]
    
    if not arquivos:
        logger.warning("Nenhum arquivo encontrado para upload na pasta processed.")
        return False
    ...
    for caminho_local in arquivos:
        ...
        s3_key = str(caminho_local.relative_to(PROCESSED_DIR)).replace("\\", "/")
...
def main() -> bool:
    ...
    enviados = enviar_processed_para_minio()
